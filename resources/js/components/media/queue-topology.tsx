...
export function QueueTopology({ className }: QueueTopologyProps) {
    return (
        <Surface tone="raised" className={cn('p-5', className)}>
            <div className="text-primary mb-4 text-[11px] font-semibold tracking-[0.22em] uppercase">Live topology</div>
            <div className="space-y-3">
                {lanes.map((lane) => (
                    <div
                        key={lane.name}
                        className="flex items-center justify-between rounded-[22px] border border-slate-900/8 bg-slate-50/80 px-4 py-3 dark:border-white/8 dark:bg-slate-900/68"
                    >
                        ...
                        <div className="text-muted-foreground text-xs tracking-[0.18em] uppercase">{lane.load}</div>
                    </div>
                ))}
            </div>
        </Surface>
